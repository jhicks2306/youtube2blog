...
import { updateVideoOutline } from '@/lib/actions';


export default function EditBlogCardContent({ video }: { video: VideoData }) {
  const router = useRouter()
  const [outline, setOutline] = useState(video.outline);
  ...
  // const handleBlog = useDebouncedCallback(async (newOutline: string) => {
  //   setSaving(true);
  //   await updateVideoOutlineWithId(newOutline);
  //   console.log('New outline saved.');
  //   setLastSaved(new Date());
    ...
    <>
        <CardContent className='flex flex-col grow'>
          <div className='grid grid-cols-5 grow'>
            <EditBlogTabs video={video} saving={saving} handleOutline={handleOutline}/>  
